...
module "application" {
  ...
  env_vars            = "${data.template_file.task_env_vars.rendered}"
}

data "template_file" "task_env_vars" {
  template = "${file("${path.module}/files/env_vars.json.tpl")}"

  vars {
    database_name = "${var.name}"
    database_user = "${var.username}"
    database_pass = "${var.password}"
  }
}
